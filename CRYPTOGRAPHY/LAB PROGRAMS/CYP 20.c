...
int main() {
    ...
    // CBC Encryption: C_i = Encrypt(P_i ^ C_{i-1})
    uint8_t prev = IV;
    for (int i = 0; i < 2; i++) {
        ciphertext[i] = sdes_encrypt(plaintext[i] ^ prev, k1, k2);
        prev = ciphertext[i];
    }

    // CBC Decryption: P_i = Decrypt(C_i) ^ C_{i-1}
    prev = IV;
    for (int i = 0; i < 2; i++) {
        decrypted[i] = sdes_decrypt(ciphertext[i], k1, k2) ^ prev;
        prev = ciphertext[i];
    }

    printf("=== Question 22: S-DES CBC Mode ===\n\n");
    printf("Block 1:\n");
    printf("  Ciphertext: "); print_binary8(ciphertext[0]); printf("\n");
    printf("  Decrypted:  "); print_binary8(decrypted[0]); printf("\n");

    printf("Block 2:\n");
    printf("  Ciphertext: "); print_binary8(ciphertext[1]); printf("\n");
    printf("  Decrypted:  "); print_binary8(decrypted[1]); printf("\n");
